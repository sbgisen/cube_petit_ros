...
int main(int argc, char** argv)
{
  ...
  ros::Publisher battery_remaining_pub =
      nh.advertise<std_msgs::Float64>("/gazebo_battery_monitor/battery_remaining", 10);
  ...
  if (serial_comminication.serialInitialize())
  {
  ...
  serial_comminication.updateBatteryStatus(g_BATTERY_STATUS);
  ...
  {
    count = 0;
    while (!serial_comminication.serialRead())
    {
      count++;
      // continue;
    }
    ...
    // 値を読み込むのは1Hz
    serial_comminication.updateBatteryStatus(g_BATTERY_STATUS);
    battery_remaining.data = g_BATTERY_STATUS[BATTERY_REMAINING];
    battery_voltage.data = g_BATTERY_STATUS[BATTERY_VOLTAGE];

    // Batteryの MAX & MIN 電圧からバッテリの残量を算出する(現在BMSからのbattery_remainingが微妙なので)
    if (custom_voltage_remain)
    {
      double battery_remaining_num =
          100 * (g_BATTERY_STATUS[BATTERY_VOLTAGE] - min_voltage) / (max_voltage - min_voltage);
    ...
    charge_current.data = g_BATTERY_STATUS[CHARGE_CURRENT];
